            ...
            while True:
                try:
                    ...
                    if "Ratelimit" in str(e) and retries < max_retries:
                        ...
                        delay = base_delay * \
                            (2 ** retries) + random.uniform(0, jitter)
                        print(
                            f"Search rate-limited. Retrying in {delay:.2f} seconds (attempt {retries}/{max_retries})...")
                        time.sleep(delay)
                    else:
    ...
    for i, query in enumerate(search_queries):
        try:
            ...
            if "Ratelimit" in str(e):
                ...
                print(
                    f"Rate limit hit. Adding extra {extra_delay:.2f}s delay...")
        ...
        if search_api == "duckduckgo":
            try:
                ...
                for i, query in enumerate(limited_queries):
                    ...
                    if i > 0:
                        print(
                            f"Adding delay between DuckDuckGo searches ({i}/{len(limited_queries)})...")
                        # 2-4 second random delay
                ...
                if "Ratelimit" in str(e):
                    print(
                        f"DuckDuckGo rate limited. Falling back to Exa API: {e}")
                    # Fall back to Exa API
                    search_api = "exa"
                    exa_params = get_search_params("exa", {})
                    # Merge any passed params
                    exa_params.update(params_to_pass)
    ...
    while True:
        try:
            ...
            if "Ratelimit" in str(e) and retries < max_retries:
                ...
                print(
                    f"DuckDuckGo search rate-limited. Retrying in {delay:.2f} seconds (attempt {retries}/{max_retries})...")
                await asyncio.sleep(delay)
            else:
                # If max retries exceeded or different error, re-raise
                raise
